...
double cos(double x){
	...
	while(i < 5){
		if(i % 2 == 0)
			sum = sum + ( pangkat(x, i*2) / faktorial(i*2) );
		else
			sum = sum - ( pangkat(x, i*2) / faktorial(i*2) );
...
int main(){
	...
	while(true){
		printf("Silahkan masukkan angka yang ingin anda masukkan ke fungsi cos(x) dalams satuan radian : \n");
		scanf("%lf", &input);
		
		printf("cos(%.2f) adalah %.2lf\n", input, cos(input));
